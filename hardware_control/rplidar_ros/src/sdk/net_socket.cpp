...
SocketAddress::SocketAddress(const char *addrString, int port,
...
SocketAddress::address_type_t SocketAddress::getAddressType() const {
  switch (
      reinterpret_cast<const sockaddr_storage *>(_platform_data)->ss_family) {
    case AF_INET:
...
int SocketAddress::getPort() const {
  switch (getAddressType()) {
    case ADDRESS_TYPE_INET:
      return (int)ntohs(
          reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_port);
    case ADDRESS_TYPE_INET6:
      return (int)ntohs(
          reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_port);
    default:
      return 0;
  }
}
...
u_result SocketAddress::setPort(int port) {
  switch (getAddressType()) {
    case ADDRESS_TYPE_INET:
      reinterpret_cast<sockaddr_in *>(_platform_data)->sin_port =
          htons((short)port);
      break;
    case ADDRESS_TYPE_INET6:
      reinterpret_cast<sockaddr_in6 *>(_platform_data)->sin6_port =
          htons((short)port);
      break;
    default:
...
u_result SocketAddress::setAddressFromString(
    const char *address_string, SocketAddress::address_type_t type) {
  int ans = 0;
  ...
  switch (type) {
    case ADDRESS_TYPE_INET:
      reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET;
      ans =
          inet_pton(AF_INET, address_string,
                    &reinterpret_cast<sockaddr_in *>(_platform_data)->sin_addr);
      break;

    case ADDRESS_TYPE_INET6:

      reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family =
          AF_INET6;
      ans = inet_pton(
          AF_INET6, address_string,
...
u_result SocketAddress::getAddressAsString(char *buffer,
  ...
  switch (net_family) {
    case AF_INET:
      ans = inet_ntop(
          net_family,
          &reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr,
          buffer, buffersize);
      break;
    ...
    case AF_INET6:
      ans = inet_ntop(
          net_family,
          &reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr,
          buffer, buffersize);
...
size_t SocketAddress::LoopUpHostName(const char *hostname,
  ...
  for (struct addrinfo *cursor = result; cursor != NULL;
       cursor = cursor->ai_next) {
    if (cursor->ai_family == ADDRESS_TYPE_INET ||
        cursor->ai_family == ADDRESS_TYPE_INET6) {
...
u_result SocketAddress::getRawAddress(_u8 *buffer, size_t bufferSize) const {
  switch (getAddressType()) {
    case ADDRESS_TYPE_INET:
      if (bufferSize < sizeof(in_addr::s_addr))
        return RESULT_INSUFFICIENT_MEMORY;

      memcpy(buffer,
             &reinterpret_cast<const sockaddr_in *>(_platform_data)
                  ->sin_addr.s_addr,
             sizeof(reinterpret_cast<const sockaddr_in *>(_platform_data)
                        ->sin_addr.s_addr));

      break;
    case ADDRESS_TYPE_INET6:
      if (bufferSize < sizeof(in6_addr::s6_addr))
        return RESULT_INSUFFICIENT_MEMORY;
      memcpy(buffer,
             reinterpret_cast<const sockaddr_in6 *>(_platform_data)
                 ->sin6_addr.s6_addr,
             sizeof(reinterpret_cast<const sockaddr_in6 *>(_platform_data)
                        ->sin6_addr.s6_addr));
...
class _single_thread StreamSocketImpl : public StreamSocket {
 public:
  StreamSocketImpl(int fd) : _socket_fd(fd) {
    assert(fd >= 0);
    int bool_true = 1;
    ::setsockopt(_socket_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&bool_true,
                 sizeof(bool_true));
  ...
  virtual ~StreamSocketImpl() { close(_socket_fd); }

  virtual void dispose() { delete this; }

  virtual u_result bind(const SocketAddress &localaddr) {
    const struct sockaddr *addr =
        reinterpret_cast<const struct sockaddr *>(localaddr.getPlatformData());
    assert(addr);
  ...
  virtual u_result getLocalAddress(SocketAddress &localaddr) {
    struct sockaddr *addr =
        reinterpret_cast<struct sockaddr *>(const_cast<void *>(
            localaddr.getPlatformData()));  // donnot do this at home...
    assert(addr);
  ...
  virtual u_result connect(const SocketAddress &pairAddress) {
    const struct sockaddr *addr = reinterpret_cast<const struct sockaddr *>(
        pairAddress.getPlatformData());
  ...
  virtual StreamSocket *accept(SocketAddress *pairAddress) {
    ...
    int pair_socket = ::accept(
        _socket_fd,
        pairAddress ? reinterpret_cast<struct sockaddr *>(
                          const_cast<void *>(pairAddress->getPlatformData()))
                    : NULL,
        (socklen_t *)&addrsize);
  ...
  virtual u_result getPeerAddress(SocketAddress &peerAddr) {
    struct sockaddr *addr =
        reinterpret_cast<struct sockaddr *>(const_cast<void *>(
            peerAddr.getPlatformData()));  // donnot do this at home...
    assert(addr);
    size_t actualsize = sizeof(sockaddr_storage);
  ...
  virtual u_result enableKeepAlive(bool enable) {
    ...
    return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE, &bool_true,
                        sizeof(bool_true))
  ...
  virtual u_result enableNoDelay(bool enable) {
    int bool_true = enable ? 1 : 0;
    return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY, &bool_true,
                        sizeof(bool_true))
               ? RESULT_OPERATION_FAIL
               : RESULT_OK;
  }
...
class _single_thread DGramSocketImpl : public DGramSocket {
 public:
  DGramSocketImpl(int fd) : _socket_fd(fd) {
    assert(fd >= 0);
    int bool_true = 1;
    ::setsockopt(_socket_fd, SOL_SOCKET, SO_REUSEADDR | SO_BROADCAST,
                 (char *)&bool_true, sizeof(bool_true));
    setTimeout(DEFAULT_SOCKET_TIMEOUT, SOCKET_DIR_BOTH);
  }

  virtual ~DGramSocketImpl() { close(_socket_fd); }
  ...
  virtual u_result sendTo(const SocketAddress &target, const void *buffer,
                          size_t len) {
    const struct sockaddr *addr =
        reinterpret_cast<const struct sockaddr *>(target.getPlatformData());
    assert(addr);
    size_t ans =
        ::sendto(_socket_fd, buffer, len, 0, addr, sizeof(sockaddr_storage));
    if (ans != (size_t)-1) {
      assert(ans == (int)len);
      return RESULT_OK;
    } else {
  ...
  virtual u_result recvFrom(void *buf, size_t len, size_t &recv_len,
                            SocketAddress *sourceAddr) {
    struct sockaddr *addr =
        (sourceAddr ? reinterpret_cast<struct sockaddr *>(
                          const_cast<void *>(sourceAddr->getPlatformData()))
                    : NULL);
    size_t source_addr_size = (sourceAddr ? sizeof(sockaddr_storage) : 0);

    size_t ans = ::recvfrom(_socket_fd, buf, len, 0, addr,
...
static inline int _socketHalFamilyToOSFamily(
...
StreamSocket *StreamSocket::CreateSocket(SocketBase::socket_family_t family) {
  ...
  StreamSocket *newborn = static_cast<StreamSocket *>(
      new rp::arch::net::StreamSocketImpl(socket_fd));
  return newborn;
}
...
DGramSocket *DGramSocket::CreateSocket(SocketBase::socket_family_t family) {
  ...
  int socket_fd = ::socket(
      socket_family, (family == SOCKET_FAMILY_RAW) ? SOCK_RAW : SOCK_DGRAM, 0);
  if (socket_fd == -1) return NULL;

  DGramSocket *newborn =
      static_cast<DGramSocket *>(new rp::arch::net::DGramSocketImpl(socket_fd));
  return newborn;
}
