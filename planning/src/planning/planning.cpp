...
void visualizeLine(const std::array<float, 3>& lineParams, cv::Scalar color) {
  for (int y = 0; y < 240; y++) {
    cv::Point p{getXByY(lineParams, y), y};
    cv::circle(debugImage, p, 3, color);
  }
}
...
Planning::Planning()
    : _nh{"planning"},
      ...
      laneToDriveCloseTo{RIGHT},
      objectState{AvoidObjectState::DONE},
      turningState{TurningState::DONE},
      leftParams{nullptr},
      rightParams{nullptr},
      currentMap{Map::RED},
      _carConfigClient{"/car_control"} {
  std::string control_topic, crossroad_topic, sign_topic, lane_topic,
      object_topic;
  ...
  _crossroadSub =
      _nh.subscribe(crossroad_topic, 1, &Planning::turnCallback, this);

  _configServer.setCallback(std::bind(&Planning::configCallback, this,
                                      std::placeholders::_1,
                                      std::placeholders::_2));
  _configCarControlServer.setCallback(
      boost::bind(&Planning::configCarControlCallback, this, _1, _2));

  _objectTimer = _nh.createTimer(ros::Duration{avoidObjectTime / 10.0f},
                                 &Planning::onObjectTimeout, this, true);
  _turnTimer = _nh.createTimer(ros::Duration{turningTime / 10.0f},
                               &Planning::onTurnTimeout, this, true);

  _setMapSrv =
      _nh.advertiseService(set_map_topic, &Planning::setMapService, this);
}
...
void Planning::planning() {
  ...
  if (!ros::param::getCached("/car_control/min_velocity", min_speed)) {
    ROS_WARN(
        "Cannot get min_velocity from ParameterServer, use default = 0.0f");
  }

  if (!ros::param::getCached("/car_control/max_velocity", max_speed)) {
    ROS_WARN(
        "Cannot get max_velocity from ParameterServer, use default = 30.0f");
  }
  // planningWhileDriveStraight(min_speed, max_speed);
  debugImage = cv::Mat::zeros(240, 320, CV_8UC3);
  if (leftParams) {
    ROS_INFO_STREAM("Have left");
    visualizeLine(leftParams->getParams(), cv::Scalar{0, 255, 0});
  } else if (rightParams) {
    ROS_INFO_STREAM("Have right");
    visualizeLine(rightParams->getParams(), cv::Scalar{255, 0, 0});
  }
...
void Planning::planningWhileDriveStraight(const float& min_speed,
                                          const float& max_speed) {
  if (!leftParams && !rightParams) {
    ROS_WARN("Both lanes not found. Not planning");
    publishMessage(lastDrivePoint, min_speed);
    return;
  }
  ...
  if (leftParams || rightParams) drivePoint = driveStraight();
  // {
...
void Planning::planningWhileTurning(const float& min_speed,
                                    const float& max_speed) {
  cv::Point drivePoint = cv::Point{165, 180};
  switch (turningState) {
    ...
    case TurningState::IS_TURNING: {
      ...
      if (isTurnable) {
        ...
        if (sign == SignState::RIGHT) {
          ...
          drivePoint = turnLeft();
        } else if (sign == SignState::FORWARD || sign == SignState::NO_LEFT ||
                   sign == SignState::NO_RIGHT) {
          return;
...
cv::Point Planning::driveCloseToLeft() {
  ...
  if (leftParams != nullptr) {
    // auto newParams = moveByGradient(leftParams->getParams(), 240, 30,
    // Direct::LEFT); // cfg 2
    auto newParams = moveByGradient(leftParams->getParams(), 240, 60,
                                    Direct::LEFT);  // cfg 1
    // auto newParams = moveByGradient(leftParams->getParams(), 240, offset,
    // Direct::LEFT);
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // leftDrive.x = getXByY(*leftParams, drivePointY, &clipMinX, &clipMaxX) +
    // 60;
    leftDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX);
  }
  return leftDrive;
}
cv::Point Planning::driveCloseToRightFromLeft() {
  ...
  if (leftParams != nullptr) {
    // auto newParams = moveByGradient(leftParams->getParams(), 240, offset,
    // Direct::LEFT); auto newParams = moveByGradient(leftParams->getParams(),
    // 240, 75, Direct::LEFT);
    auto newParams =
        moveByGradient(leftParams->getParams(), 240, 135, Direct::LEFT);
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // leftDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX) +
    // 135;
    leftDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX);
...
cv::Point Planning::driveStraightFromLeft() {
  ...
  cv::Point leftDrive{0, drivePointY};
  if (leftParams != nullptr) {
    auto newParams = moveByGradient(leftParams->getParams(), 240, 85,
                                    Direct::LEFT);  // cfg 1
    // auto newParams = moveByGradient(leftParams->getParams(), 240, 50,
    // Direct::LEFT); // cfg 2 auto newParams =
    // moveByGradient(leftParams->getParams(), 240, offset, Direct::LEFT);
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // leftDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX) + 85;
    leftDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX);
  }
  return leftDrive;
}
...
cv::Point Planning::driveStraightFromRight() {
  ...
  if (rightParams != nullptr) {
    auto newParams = moveByGradient(rightParams->getParams(), 240, 85,
                                    Direct::RIGHT);  // cfg 1
    // auto newParams = moveByGradient(rightParams->getParams(), 240, 50,
    // Direct::RIGHT) // cfg 2;
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // rightDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX) -
    // 85;
    rightDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX);
  }
  return rightDrive;
...
cv::Point Planning::driveCloseToRight() {
  ...
  cv::Point rightDrive{319, drivePointY};
  if (rightParams != nullptr) {
    // auto newParams = moveByGradient(rightParams->getParams(), 240, offset,
    // Direct::RIGHT);
    auto newParams = moveByGradient(rightParams->getParams(), 240, 60,
                                    Direct::RIGHT);  // cfg 1
    // auto newParams = moveByGradient(rightParams->getParams(), 240, 30,
    // Direct::RIGHT) // cfg 2;
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // rightDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX) -
    // 60;
...
cv::Point Planning::driveCloseToLeftFromRight() {
  ...
  if (rightParams != nullptr) {
    // auto newParams = moveByGradient(rightParams->getParams(), 240, offset,
    // Direct::RIGHT);
    auto newParams = moveByGradient(rightParams->getParams(), 240, 135,
                                    Direct::RIGHT);  // cfg 1
    // auto newParams = moveByGradient(rightParams->getParams(), 240, 75,
    // Direct::RIGHT); // cfg 2
    visualizeLine(newParams, cv::Scalar{0, 0, 255});
    // rightDrive.x = getXByY(newParams, drivePointY, &clipMinX, &clipMaxX) -
    // 135;
...
cv::Point Planning::driveStraight() {
  ...
  ROS_DEBUG_COND(object != 0, "OBJECT STATE = %d",
                 static_cast<int>(objectState));
  switch (objectState) {
...
void Planning::configCallback(planning::PlanningConfig& config,
                              uint32_t level) {
  turningTime = config.turning_time;
  avoidObjectTime = config.avoid_time;
  laneToDriveCloseTo =
      config.lane_close - 1;  // config unable to set negative value
  offset = config.offset_lane;
  drivePointY = config.drivepoint_y;
}
...
void Planning::requestResetLane(int lane) {
  ...
  if (_resetLaneClient.call(srv)) {
    ...
    if (srv.request.lane == srv.request.LEFT) {
      std::copy(srv.response.left_params.begin(),
                srv.response.left_params.end(), params.begin());
      leftParams = std::make_shared<LineParams>(params);
    } else if (srv.request.lane == srv.request.RIGHT) {
      std::copy(srv.response.right_params.begin(),
                srv.response.right_params.end(), params.begin());
...
bool Planning::requestRecover(int lane) {
  ...
  if (_recoverClient.call(srv)) {
    ...
    if (srv.response.params.size() > 0) {
      ROS_DEBUG_STREAM("Request Recover Receive results OK, size = "
                       << srv.response.params.size());
      auto p = LineParams::ImplType();
      std::copy(srv.response.params.begin(), srv.response.params.end(),
                p.begin());
...
bool Planning::requestIsAbleToTurn(int direction) {
  ros::ServiceClient client =
      _nh.serviceClient<cds_msgs::IsTurnable>("/lane_detect/IsTurnable");
...
bool Planning::setMapService(cds_msgs::SetMap::Request& req,
                             cds_msgs::SetMap::Response& resp) {
  ROS_INFO("TRIGGER SET MAP SERVICE");
  if (req.map != static_cast<int>(currentMap)) {
    setValuesBasedOnMap(static_cast<Map>(req.map));
  }
  ROS_INFO("TRIGGER SET MAP SERVICE DONE");
  if (req.map == req.RED) {
...
void Planning::setValuesBasedOnMap(Map map) {
  // {
  //   // Set laneToDriveCloseTo
  //   dynamic_reconfigure::ReconfigureRequest srv_req;
  //   dynamic_reconfigure::ReconfigureResponse srv_resp;
  //   dynamic_reconfigure::Config config;
  //   dynamic_reconfigure::IntParameter param;
  //   param.name = "lane_close";
  //   param.value = (map == Map::RED) ? 0 : 2;
  //   config.ints.push_back(param);
  //   srv_req.config = config;
  //   ROS_INFO("CALL SERVICE");
  //   if (ros::service::call("/planning/set_parameters", srv_req, srv_resp)) {
  //     ROS_INFO_STREAM("Set lane to " << laneToDriveCloseTo);
  //   } else {
  //     ROS_ERROR("Cannot set lane to drive closed to based on map!!");
  //   }
  // }
  ...
  {
    _carConfig.kCU = 2500;
    _carConfig.PU = (map == Map::RED) ? 1000 : 500;
    _carConfigClient.setConfiguration(_carConfig);
  }
}

void Planning::configCarControlCallback(car_control::CarControlConfig& config,
                                        uint32_t level) {
  _carConfig = config;
}