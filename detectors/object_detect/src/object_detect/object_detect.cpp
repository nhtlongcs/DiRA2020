...
enum OBJ_STRATEGY { ON_RAW_BINARY = 0, ON_KMEAN, LEN };
...
ObjectDetect::ObjectDetect()
  ...
  std::string transport_hint =
      _nh.param<std::string>("transport_hint", "compressed");
  std::string objectTopic;
  ROS_ASSERT(ros::param::get("/object_topic", objectTopic));

  _serverConfig.setCallback(
      boost::bind(&ObjectDetect::configCallback, this, _1, _2));
  ...
  _depthSub =
      _it.subscribe(depth_topic, 1, &ObjectDetect::updateDepthCallback, this,
                    image_transport::TransportHints{transport_hint});
  _binarySub =
      _it.subscribe(lane_seg_topic, 1, &ObjectDetect::updateBinaryCallback,
                    this, image_transport::TransportHints{transport_hint});

  // cv::namedWindow(CONF_OBJ_WINDOW, cv::WINDOW_GUI_NORMAL);
  // cv::createTrackbar("clusterCount", CONF_OBJ_WINDOW, &kCluster, 10);

  // cv::createTrackbar("strategy", CONF_OBJ_WINDOW, &strategy,
  // OBJ_STRATEGY::LEN - 1);
  ...
  // // cv::createTrackbar("objectROI_w", CONF_OBJ_WINDOW, &objectROIRect.width,
  // 320); cv::createTrackbar("objectROI_h", CONF_OBJ_WINDOW,
  // &objectROIRect.height, 240); cv::createTrackbar("objectROI_offsetTop",
  // CONF_OBJ_WINDOW, &objectROI_offsetTop, 100);

  // cv::createTrackbar("Detect threshold", CONF_OBJ_WINDOW, &detectThreshold,
  // 100); // percents cv::createTrackbar("DiffToKnowLeftRight",
  // CONF_OBJ_WINDOW, &diffDirectPercent, 100); // percents

  // cv::createTrackbar("DepthThresholdMin", CONF_OBJ_WINDOW,
  // &depthThresholdMin, 255); // percents
  // cv::createTrackbar("DepthThresholdMax", CONF_OBJ_WINDOW,
  // &depthThresholdMax, 255); // percents
...
void ObjectDetect::configCallback(object_detect::ObjectDetectConfig &config,
                                  uint32_t level) {
...
void ObjectDetect::updateBinaryCallback(const sensor_msgs::ImageConstPtr &msg) {
  ...
  try {
    ...
    ROS_ERROR("Could not convert from '%s' to '%s'.", msg->encoding.c_str(),
...
//     cv::inRange(hsv, cv::Scalar(configObjectmin[0], configObjectmin[1],
//     configObjectmin[2]),
//                     cv::Scalar(configObjectmax[0], configObjectmax[1],
//                     configObjectmax[2]), mask);
...
//     float percent = cv::countNonZero(fgMask) * 100.0f/ (fgMask.rows *
//     fgMask.cols); std::cout << "percent :" << percent << std::endl; if (
//     percent >= detectThreshold)
//     {
...
int ObjectDetect::estimateDirect(const cv::Mat &binaryROI) {
  ...
  cv::Rect objectROI_right{binaryROI.cols / 2, 0, binaryROI.cols / 2,
                           binaryROI.rows};
  ...
  float percentLeft =
      countLeft * 100.0f / (ImageLeft.rows * ImageLeft.cols);  // 0-100%
  float percentRight =
      countRight * 100.0f / (ImageRight.rows * ImageRight.cols);  // 0-100%

  ROS_DEBUG("Percent left = %.2f, right = %.2f, diff = %.2f", percentLeft,
            percentRight, percentLeft - percentRight);
  if (abs(percentLeft - percentRight) > diffDirectPercent) {
    return (percentLeft > percentRight) ? static_cast<int>(LEFT)
...
int ObjectDetect::getDirectOnRawBinary(const cv::Rect &objectROI) {
  ...
  cv::inRange(depthThresholded, cv::Scalar{depthThresholdMin * 1.0},
              cv::Scalar{depthThresholdMax * 1.0}, depthThresholded);
...
int ObjectDetect::getDirectOnKmean(const cv::Rect &objectROI) {
  ...
  cv::Mat mask;
  cv::inRange(kmeanImage, cv::Scalar{depthThresholdMin * 1.0},
              cv::Scalar{depthThresholdMax * 1.0}, mask);
...
int ObjectDetect::detectOneFrame() {
  if (this->binary.empty() || this->depth.empty()) {
    ROS_ERROR(
        "Binary image empty or depth empty. Make sure subscribing right topic");
    return 0;
  }

  Hough(this->binary);
  ROS_DEBUG("objectROI = %d, %d, %d, %d", objectROIRect.x, objectROIRect.y,
            objectROIRect.width, objectROIRect.height);
...
void ObjectDetect::Hough(const cv::Mat &binary) {
  ...
  cv::HoughLinesP(binary, lines, 1, CV_PI / 180, votes, minLinlength,
                  maxLineGap);
  ...
  cv::circle(HoughTransform, cv::Point(midX, midY), 3, cv::Scalar(0, 0, 255),
             -1);
  cv::circle(HoughTransform, cv::Point(offsetX, offsetY), 3,
             cv::Scalar(0, 255, 0), -1);

  if ((lineL[0] != 0) && (lineR[0] != 0.0f)) {
    int left_most_x, right_most_x;
    // ROS_INFO("lineL = [%.2f, %.2f], lineR = [%.2f, %.2f]", lineL[0],
    // lineL[1], lineR[0], lineR[1]);
    ...
    // ROS_INFO("1: left_most_x = %d, right_most_x = %d", left_most_x,
    // right_most_x);

    left_most_x =
        std::min(std::max(left_most_x - offsetROI_x, 0), binary.cols - 1);
    right_most_x =
        std::min(std::max(right_most_x + offsetROI_x, 0), binary.cols - 1);

    // ROS_INFO("2: left_most_x = %d, right_most_x = %d", left_most_x,
    // right_most_x);
...
void ObjectDetect::drawLine(float slope, float y_intercept,
                            cv::Mat &HoughTransform) {
  float y0 = 240.0;
  float y1 = 1.0;
  float x0 = (y0 - y_intercept) / slope;
  float x1 = (y1 - y_intercept) / slope;
  cv::line(HoughTransform, cv::Point(x0, y0), cv::Point(x1, y1),
...
void ObjectDetect::update() {
  ...
  // int cntLeft = std::count(objectHistories.begin(), objectHistories.end(),
  // -1); int cntRight = std::count(objectHistories.begin(),
  // objectHistories.end(), 1); int cntNone =
  // std::count(objectHistories.begin(), objectHistories.end(), 0);
