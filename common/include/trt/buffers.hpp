...
static void constexpr cudaCheck(cudaError_t const& status) {
  if (status != cudaSuccess) {
    trt_logger::gLogError << "Cuda failure: " << status << std::endl;
    abort();
  }
}
...
static int volume(nvinfer1::Dims const& d) {
 ...
 public:
  ...
  BufferManager(std::shared_ptr<nvinfer1::ICudaEngine> engine,
    ...
    for (int i = 0; i < mEngine->getNbBindings(); ++i) {
      ...
      if (vecDim != -1) {  // i.e., 0 != lgScalarsPerVector
