...
void process() {
  ...
  cv::inRange(depthImage, cv::Scalar{double(minDepth)},
              cv::Scalar{double(maxDepth)}, depthThresholded);
  ...
  if (area < (ratio / 100.0f)) {
    ROS_ERROR_COND(!is_stop_writing, "Drop frame due to area = %.2f < %.2f",
                   area, ratio / 100.0f);
  } else if (!is_stop_writing) {
    std::string filename =
        path + "/frame_" + std::to_string(frame_count) + std::string{".jpg"};
    // write image
    if (!cv::imwrite(filename, colorImage)) {
      ROS_ERROR_COND(!is_stop_writing, "Cannot write image: %s",
                     filename.c_str());
    }
    // write box
...
int main(int argc, char **argv) {
  ...
  image_transport::Subscriber colorSub =
      it.subscribe("/camera/rgb/image_raw", 1, &colorCallback);
  image_transport::Subscriber depthSub =
      it.subscribe("/camera/depth/image", 1, &depthCallback);
  pBackSub = cv::createBackgroundSubtractorMOG2();

  std::string classname;
  const std::unordered_set<std::string> valid_names = {
      "forward", "left", "right", "notleft", "notright", "stop"};
  if (!private_nh.getParam("class", classname) ||
      valid_names.find(classname) == valid_names.end()) {
    ROS_FATAL("INVALID NAME");
    exit(EXIT_FAILURE);
  }

  path = ros::package::getPath("data_collection") + "/images/" + classname;
  file = std::ofstream{path + "/bounding_box.csv",
