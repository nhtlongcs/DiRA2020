...
CarControl::CarControl() : _nh{"car_control"}, _configServer{_nh} {
  _configServer.setCallback(
      boost::bind(&CarControl::configCallback, this, _1, _2));

  carPos.x = 165;
  carPos.y = 230;

  std::string speed_topic, steer_topic, control_topic, max_speed_srv,
      min_speed_srv;
  ...
  control_subscriber =
      _nh.subscribe(control_topic, 1, &CarControl::driveCallback, this);
  _incDecMaxVelSrv = _nh.advertiseService(
      max_speed_srv, &CarControl::incDecMaxVelSrvCallback, this);
  _incDecMinVelSrv = _nh.advertiseService(
      min_speed_srv, &CarControl::incDecMinVelSrvCallback, this);
}

void CarControl::configCallback(car_control::CarControlConfig& config,
                                uint32_t level) {
  kCU = config.kCU;
  PU = config.PU;
  // kP = config.P;
...
bool CarControl::incDecMaxVelSrvCallback(cds_msgs::IncDecSpeed::Request& req,
                                         cds_msgs::IncDecSpeed::Response& res) {
...
bool CarControl::incDecMinVelSrvCallback(cds_msgs::IncDecSpeed::Request& req,
                                         cds_msgs::IncDecSpeed::Response& res) {
  if (req.mode == req.MODE_INC) {
    minVelocity += 5;
  } else if (req.mode == req.MODE_DEC) {
...
void CarControl::driveCallback(const geometry_msgs::Twist& msg) {
  ...
  // speed.data = fabs(error) < 1 ? maxVelocity : (velocity - fabs(error) *
  // 0.35);
